...
function commit -d "Gitnow: Commit changes gitmoji-cli" -w 'gitmoji -c'
    if not __gitnow_is_git_repository
...
function move -d "GitNow: Switch from current branch to another but stashing uncommitted changes" -a args
    ...
    for v in $argv
        switch $v
            ...
            case '*'
                if begin
                        test -n "$v_remote"; and not test -n "$v_remote_v"
                    end
    ...
    if begin
            test -n "$v_prev"; or [ "$args" = - ]
        end
        if begin
                test -z "$g_current_branch"; or [ "$g_current_branch" = (__gitnow_current_branch_name) ]
            end
    ...
    # Branch was not found
    if begin
            test $v_found -eq 0; and test $v_fetched -eq 0
        end
    ...
    if begin
            test $v_uncommited; and not test -n "$v_no_apply_stash"
        end
